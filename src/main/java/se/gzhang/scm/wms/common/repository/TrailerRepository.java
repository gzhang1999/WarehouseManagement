...
public interface TrailerRepository extends JpaRepository<Trailer, Integer>, JpaSpecificationExecutor<Trailer> {
    ...
    List<Trailer> findByTrailerNumber(String trailerNumber);

    List<Trailer> findByTrailerType(TrailerType trailerType);

    List<Trailer> findByDriver(String driver);

    List<Trailer> findByLicensePlate(String licensePlate);

    List<Trailer> findByTrailerState(TrailerState trailerState);
}
