...
public class ReserveShippingStageLocationByAnythingStrategy extends DefaultShippingStageLocationReserveStrategy {

    public ReserveShippingStageLocationByAnythingStrategy(LocationService locationService, SalesOrderService salesOrderService) {
    ...
    public boolean isReservable(Location location, SalesOrder salesOrder) {
        if (location.getVolume() - location.getUsedVolume() > salesOrderService.getSalesOrderVolume(salesOrder, location.getArea().getVolumeType())) {
            return true;
        }
        return false;
    }
